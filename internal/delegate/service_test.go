package delegate

import (
	"context"
	"encoding/json"
	"fmt"
	"testing"

	"github.com/goverland-labs/goverland-datasource-snapshot/pkg/gnosis"
)

func TestService_GetDelegates(t *testing.T) {
	txt := `{
    "name": "split-delegation",
    "params": {
      "backendUrl": "https://delegate-api.gnosisguild.org",
      "strategies": [
        {
          "name": "erc20-balance-of",
          "params": {
            "symbol": "SAFE",
            "address": "0x5aFE3855358E112B5647B952709E6165e1c1eEEe",
            "decimals": 18
          },
          "network": 1
        },
        {
          "name": "safe-vested",
          "params": {
            "symbol": "SAFE (vested)",
            "claimDateLimit": "2022-12-27T10:00:00+00:00",
            "allocationsSource": "https://safe-claiming-app-data.gnosis-safe.io/allocations/1/snapshot-allocations-data.json"
          },
          "network": 1
        },
        {
          "name": "contract-call",
          "params": {
            "symbol": "SAFE (locked)",
            "address": "0x0a7CB434f96f65972D46A5c1A64a9654dC9959b2",
            "decimals": 18,
            "methodABI": {
              "name": "getUserTokenBalance",
              "type": "function",
              "inputs": [
                {
                  "name": "holder",
                  "type": "address",
                  "internalType": "address"
                }
              ],
              "outputs": [
                {
                  "name": "",
                  "type": "uint96",
                  "internalType": "uint96"
                }
              ],
              "stateMutability": "view"
            }
          },
          "network": 1
        }
      ],
      "totalSupply": 1000000000
    },
    "network": "1"
  }`

	type marshalStrategy struct {
		Name    string                 `json:"name"`
		Network string                 `json:"network"`
		Params  map[string]interface{} `json:"params"`
	}

	var ms marshalStrategy
	if err := json.Unmarshal([]byte(txt), &ms); err != nil {
		t.Fatalf("Unmarshal error: %v", err)
	}

	msJson, err := json.Marshal(ms)
	if err != nil {
		t.Fatalf("Marshal error: %v", err)
	}
	type args struct {
		req GetDelegatesParams
	}
	tests := []struct {
		name    string
		args    args
		want    []Delegate
		wantErr bool
	}{
		{
			name: "Test GetDelegates",
			args: args{
				req: GetDelegatesParams{
					Dao:       "safe.ggtest.eth",
					Strategy:  msJson,
					By:        "power",
					Addresses: nil,
					Limit:     20,
					Offset:    0,
				},
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := &Service{
				gnosisSDK: gnosis.NewSDK(),
			}
			got, err := s.GetDelegates(context.Background(), tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetDelegates() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			indent, _ := json.MarshalIndent(got, "", " ")
			fmt.Println(string(indent))
		})
	}
}

func TestService_GetDelegateProfile(t *testing.T) {
	txt := `{
    "name": "split-delegation",
    "params": {
      "backendUrl": "https://delegate-api.gnosisguild.org",
      "strategies": [
        {
          "name": "erc20-balance-of",
          "params": {
            "symbol": "SAFE",
            "address": "0x5aFE3855358E112B5647B952709E6165e1c1eEEe",
            "decimals": 18
          },
          "network": 1
        },
        {
          "name": "safe-vested",
          "params": {
            "symbol": "SAFE (vested)",
            "claimDateLimit": "2022-12-27T10:00:00+00:00",
            "allocationsSource": "https://safe-claiming-app-data.gnosis-safe.io/allocations/1/snapshot-allocations-data.json"
          },
          "network": 1
        },
        {
          "name": "contract-call",
          "params": {
            "symbol": "SAFE (locked)",
            "address": "0x0a7CB434f96f65972D46A5c1A64a9654dC9959b2",
            "decimals": 18,
            "methodABI": {
              "name": "getUserTokenBalance",
              "type": "function",
              "inputs": [
                {
                  "name": "holder",
                  "type": "address",
                  "internalType": "address"
                }
              ],
              "outputs": [
                {
                  "name": "",
                  "type": "uint96",
                  "internalType": "uint96"
                }
              ],
              "stateMutability": "view"
            }
          },
          "network": 1
        }
      ],
      "totalSupply": 1000000000
    },
    "network": "1"
  }`

	type marshalStrategy struct {
		Name    string                 `json:"name"`
		Network string                 `json:"network"`
		Params  map[string]interface{} `json:"params"`
	}

	var ms marshalStrategy
	if err := json.Unmarshal([]byte(txt), &ms); err != nil {
		t.Fatalf("Unmarshal error: %v", err)
	}

	msJson, err := json.Marshal(ms)
	if err != nil {
		t.Fatalf("Marshal error: %v", err)
	}
	type args struct {
		req GetDelegateProfileParams
	}
	tests := []struct {
		name    string
		args    args
		want    []Delegate
		wantErr bool
	}{
		{
			name: "Test GetDelegateProfile",
			args: args{
				req: GetDelegateProfileParams{
					Dao:      "safe.ggtest.eth",
					Strategy: msJson,
					Address:  "0x7697cAB0e123c68d27d7D5A9EbA346d7584Af888",
				},
			},
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			s := &Service{
				gnosisSDK: gnosis.NewSDK(),
			}
			got, err := s.GetDelegateProfile(context.Background(), tt.args.req)
			if (err != nil) != tt.wantErr {
				t.Errorf("GetDelegateProfile() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			indent, _ := json.MarshalIndent(got, "", " ")
			fmt.Println(string(indent))
		})
	}
}
